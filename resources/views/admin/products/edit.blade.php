...
@section('content')
    <div class="main-content-wrap">
        ...
        <form class="tf-section-2 form-add-product" action="{{ route('admin.products.update', $product) }}" method="POST"
            ...
            <div class="wg-box">
                ...
                <div class="lang-content" data-lang-content="en">
                    ...
                    <x-form.textarea className="shortdescription mb-20" label="{{ __('English Short Description') }}"
                        required name="short_description_en" placeholder="{{ __('Enter the english short description') }}"
                ...
                <div class="lang-content hidden" data-lang-content="ar">
                    ...
                    <x-form.textarea className="shortdescription mb-10" label="{{ __('Arabic Short Description') }}"
                        required name="short_description_ar" placeholder="{{ __('Enter the arabic short description') }}"
                        value="{{ old('short_description_ar', $product->short_description_ar) }}" />
                ...
                <div class="gap22 cols mb-10 mt-20">
                    <x-form.select className="category" label="{{ __('Category') }}" required name="category_id"
                        :options="$categories" oldValue="{{ old('category_id', $product->category->id) }}" />
                    <x-form.select className="brand" label="{{ __('Brand') }}" required name="brand_id" :options="$brands"
                        oldValue="{{ old('brand_id', $product->brand_id) }}" />
                </div>
            </div>
            <div class="wg-box">
                <x-form.file label="{{ __('Upload image') }}" required name="image" :image="$product->image_path" />
                <x-form.file label="{{ __('Upload Gallery Images') }}" name="gallery[]" :multiple="true"
                    :images="$product->gallery->map(
                        fn($img) => ['id' => $img->id, 'url' => asset('storage/' . $img->path)],
                    )" />
                ...
                <div class="cols gap22">
                    <x-form.input className="name" label="{{ __('SKU') }}" required type="text"
                        placeholder="{{ __('Enter SKU') }}" name="sku" value="{{ old('sku', $product->sku) }}" />
                ...
                <div class="cols gap22">
                    <x-form.select className="name" label="{{ __('Stock') }}" required name="in_stock"
                        :options="$stocks" oldValue="{{ old('in_stock', $product->in_stock) }}" />
                    <x-form.select className="name" label="{{ __('Featured') }}" required name="featured"
                        :options="$featuredOptions" oldValue="{{ old('featured', $product->featured) }}" />
                </div>
                <div class="cols gap10">
                    <button class="tf-button w-full" type="submit">{{ __('Edit product') }}</button>
                </div>
            </div>
        </form>
